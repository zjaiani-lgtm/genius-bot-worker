...
def run_startup_sync() -> bool:
    ...
    - DEMO:
        - Always OK (positions handled virtually)
    - TESTNET / LIVE:
        - 1) Public connectivity check (no API key required)
        - 2) Private connectivity check (requires API key)
    ...
    try:
        ...
        log_info(f"STARTUP_SYNC: {mode} -> DIAG base={diag.get('base_public')} key_len={diag.get('key_len')} secret_len={diag.get('secret_len')} key_prefix={diag.get('key_prefix')}")

        # 3.1 Public check (endpoint correctness)
        ...
        # 3.2 Private check (keys correctness)
