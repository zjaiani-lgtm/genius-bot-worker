...
def _calc_pnl(side: str, entry_price: float, close_price: float, size: float) -> float:
    """
    Simplified DEMO PnL:
      LONG  -> (close - entry) * size
      SHORT -> (entry - close) * size
    """
    if side == "LONG":
...
def execute_signal(signal: dict) -> None:
    ...
    # ---- Idempotency gate (for BOTH TRADE & CLOSE) ----
    ...
    if verdict == "TRADE":
        ...
        direction = (execution.get("direction") or "").upper()
        ...
        if not symbol or direction not in ("LONG", "SHORT") or position_size is None:
            log_warning(f"Engine: missing/invalid TRADE fields -> NO EXECUTION | id={signal_id}")
            return

        # Secondary safety: only one open position per symbol
        ...
        if fill.get("status") != "FILLED":
            log_warning(f"Engine: entry not FILLED -> NO DB WRITE | id={signal_id}")
        ...
        log_info(
            f"DEMO TRADE OPENED | id={signal_id} | {symbol} {side} size={position_size} price={fill['price']}"
        )
        return
    ...
    if verdict == "CLOSE":
        ...
        if not symbol or close_price is None:
            log_warning(f"Engine: CLOSE missing symbol/price -> NO EXECUTION | id={signal_id}")
            return
        ...
        if pos is None:
            ...
            log_event(
                "CLOSE_BLOCKED",
                f"id={signal_id} reason=NO_OPEN_POSITION symbol={symbol}",
            )
            return
        ...
        if fill.get("status") != "FILLED":
            log_warning(f"Engine: close not FILLED -> NO DB WRITE | id={signal_id}")
        ...
        log_event(
            ...
            f"id={signal_id} pos_id={position_id} {symbol} {side} "
            f"entry={entry_price} close={close_price} size={size} pnl={pnl}",
        )

        log_info(
            f"DEMO POSITION CLOSED | id={signal_id} | pos_id={position_id} "
            f"{symbol} {side} pnl={pnl}"
        )
        return
